...
import { getPersonalChannelsStatus } from "@/modules/personal-channels/service";
...
export async function GET() {
  ...
  try {
    return okResponse(await getPersonalChannelsStatus(auth.user));
  } catch (error) {
    ...
    return errorResponse(
      "INTERNAL_ERROR",
      "Failed to load personal channel status.",
      500,
    );
  }
}
