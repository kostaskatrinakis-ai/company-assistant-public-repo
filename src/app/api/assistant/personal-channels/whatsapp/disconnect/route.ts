...
import { disconnectPersonalWhatsApp } from "@/modules/personal-channels/service";
...
export async function POST() {
  ...
  try {
    return okResponse(await disconnectPersonalWhatsApp(auth.user));
  } catch (error) {
    ...
    return errorResponse(
      "INTERNAL_ERROR",
      "Failed to disconnect WhatsApp personal session.",
      500,
    );
  }
}
